...
#This is a Non-Convex Mixed Integer Quadratically Constrained Programming (Non-Convex MIQCP) model 
#Scenarios
...
#Set dyanmic learning and thresolds length
#Thresholds = "Latemover"#Equal, Equal5 ,Firstmover ,Latemover  Early, Late, scurve
...
Cap_0 = [0 0]
if dynamic_learning=="TRUE"
    All_results_folder = "Dynamic_Learning"
elseif dynamic_learning =="FALSE"
    All_results_folder = "Static_Learning"
end
...
if dynamic_learning=="TRUE"
    ...
    #add fuel cost threshold driver # Add a constraint to calibrate the initial fuel cost to our 2020 estimates
    # Set the initial parameter values and thresholds
    function generate_values(step)
        values = Int64[]
        current_value = 0
    ...
    #Set Scenarios
    #if Thresholds == "Firstmover"
    #    steps = 2000 
    #elseif Thresholds =="Latemover"
    #    steps = 10000
    #elseif Thresholds == "Equal"
    #    steps= 5000
    #end
    thresholds = generate_values(steps)
    print(thresholds)
    #steady_shape1 = [1, 0.96, 0.92, 0.88, 0.84, 0.80, 0.76, 0.72, 0.68, 0.64, 0.6]
    steady_shape2 = [1, 0.92, 0.84, 0.76, 0.68, 0.60, 0.52, 0.44, 0.36, 0.28, 0.2]
    #steady_shape3 = [1, 0.98, 0.96, 0.94, 0.92, 0.90, 0.88, 0.86, 0.84, 0.82, 0.8]
    #s_shape1 = [1,0.995,0.99,0.95,0.775,0.6,0.996,0.997,0.998,0.99,0.99]
    #s_shape2 = [1,0.995,0.99,0.95,0.875,0.8,0.996,0.997,0.998,0.99,0.99]
    #s_shape3 = [1,0.995,0.99,0.95,0.575,0.2,0.996,0.997,0.998,0.99,0.99]

    param_values_exogenous =steady_shape2
    ...
    @variable(Shipping_stock, experience_gathered[1:F,1:Y])
    J = length(thresholds)
    ...
    for f in eH2based
        ...
        #@constraint(Shipping_stock, [j=1:J,y=1:Y-1],experience_gathered[f,y+1]==sum(TotFuel[f,y] for y in 1:y))
        # Add constraints to active the respective binary for the threshold reached
        #@constraint(Shipping_stock, [j=1:J,y=1:Y],experience_gathered[f,y]>=thresholds[j]*b[f,y,j])
        @constraint(Shipping_stock, [j=1:J,y=1:Y],TotFuel[f,y]>=thresholds[j]*b[f,y,j])
        # Add constraints to ensure only one threshold is triggered
        @constraint(Shipping_stock, [y=1:Y], sum(b[f,y,j] for j in 1:J)== 1)
        # Add a constraint to update the parameter value based on the threshold that has been reached in each year
        @constraint(Shipping_stock, [y=1:Y],learning_parameter[f,y] == sum(b[f,y,j]* param_values_exogenous[j] for j in 1:J))  
        # Add a constraint to calibrate the initial fuel cost to our 2020 estimates
        @constraint(Shipping_stock,[y=1], fuel_cost_dynamic[f, y] == fuel_cost_0[f])  
        #@constraint(Shipping_stock,[f = 1:7 & 11:12 & 14:16,y=1:Y], fuel_cost_dynamic[f, y] == fuel_cost_0[f])  
        @constraint(Shipping_stock,[y=2:Y], fuel_cost_dynamic[f, y] == fuel_cost_0[f]-(fuel_cost_0[f]*(1-learning_parameter[f,y])))  
        @constraint(Shipping_stock,[y=2:Y], fuel_cost_dynamic[f, y] <= fuel_cost_dynamic[f,y-1]) 
    end
    
    for f in Fossil_based  # Add a constraint to calibrate the initial fuel cost to our 2020 estimates
            @constraint(Shipping_stock,[y=1:Y], fuel_cost_dynamic[f, y] == fuel_cost_0[f])  
    end
elseif dynamic_learning != "TRUE"
    @constraint(Shipping_stock,[y=1:Y,f=1:F], fuel_cost_dynamic[f, y] == fuel_cost[f,y])  
...
if dynamic_learning=="TRUE"
    ...
    experience_gathered_data = zeros(F,Y)
    for f=1:F
        for y = 1:Y
            experience_gathered_data[f,y] = sum(JuMP.value.(experience_gathered[f,y]))
        end
    end
end
...
experience = DataFrame([Year transpose(experience_gathered_data)],:auto)
end
...
CSV.write(joinpath(Results_folder, "experience.csv"), experience)
end
