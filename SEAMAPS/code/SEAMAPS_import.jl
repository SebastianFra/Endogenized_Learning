...
VLSFO = findfirst(x -> x == "VLSFO/HFOsc", Data_fuel_cost[2:end, 1])
MDO = findfirst(x -> x == "MDO", Data_fuel_cost[2:end, 1])
MGO = findfirst(x -> x == "MGO", Data_fuel_cost[2:end, 1])
LNG = findfirst(x -> x == "LNG", Data_fuel_cost[2:end, 1])
LPG = findfirst(x -> x == "LPG", Data_fuel_cost[2:end, 1])
METgrey = findfirst(x -> x == "MET-grey", Data_fuel_cost[2:end, 1])
METblue = findfirst(x -> x == "MET-blue", Data_fuel_cost[2:end, 1])
AMMgrey = findfirst(x -> x == "AMM-grey", Data_fuel_cost[2:end, 1])
AMMblue= findfirst(x -> x == "AMM-blue", Data_fuel_cost[2:end, 1])
Gap= findfirst(x -> x == "Gap", Data_fuel_cost[2:end, 1])

eH2based = [NH3_green BioMeOH PSMeOH DACMeOH]  ; nH2based = length(eH2based)
Fossil_based = [VLSFO MDO MGO LNG LPG METgrey METblue AMMgrey AMMblue RefPO LBG Gap]  ; nFossil_based = length(Fossil_based)
...
if Ambition == "Realistic"
    ...
    EL_TTW_50= Data_emissions_cap_TTW[2:end, 2:end]
end
#create fuel tax
if Taxed == "WTW"
    if MBM != "CO2_tax_flat"
...
elseif MBM == "Global_cap"
    for y = 1:Y
        EL_WTW_99_adj[y] = EL_WTW_99_adj[y] 
    end
    for f = 1:F, y = 1:Y
        fuel_tax[f, y] = fuel_tax[f, y] * 0
    end
